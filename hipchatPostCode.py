...
ret = worker.postCode(name=name, runFilename="hipchatTask.py", zipFilename="hipchatTask.zip")
print str(ret)
...
for project in projects: 
  ...
  try:
    ...
    continue
  #worker.deleteCode(code_id=code_id)
  codes = worker.getCodes()
  for code in codes:
    if (code_id == code['id']):
...
worker.setProject(project_id)  # make this the default...

details = worker.getProjectDetails(project_id=project_id)
...
codes = worker.getCodes(project_id)
...
details = worker.getCodeDetails(code_id=code_id)
...
schedule_id = worker.postSchedule(name=name, 10)
